module Blazer
  class Check < Record
    ...
    def fix_emails
      ...
      self.emails = emails.strip.gsub(/[;\s]/, ',').gsub(/,+/, ', ') if emails.present?
    end

    def validate_emails
      errors.add(:base, 'Invalid emails') unless split_emails.all? { |e| e =~ /\A\S+@\S+\.\S+\z/ }
    end

    def validate_variables
      errors.add(:base, "Query can't have variables") if query.variables.any?
    end
  end
end
